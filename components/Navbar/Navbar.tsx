...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (blackDivRef.current) {
        const blackDivBottom =
          blackDivRef.current.getBoundingClientRect().bottom;
  ...
  const searchProducts = async (query: string) => {
    ...
    try {
      const response = await fetch(
        `/api/products?search=${encodeURIComponent(query)}&archived=false`
      );
  ...
  return createPortal(
    <Suspense fallback={<div>Loading...</div>}>
      <section className="font-unbounded z-100">
        <div
          ref={blackDivRef}
          className="bg-black p-3 text-white text-center text-sm md:text-base"
        >
          ...
          <div className="w-11/12 mx-auto flex items-center justify-between">
            <div className="relative flex items-center md:gap-4">
              <div
                ...
                className="relative w-10 z-[9999]"
                role="button"
              ...
              >
                <Image
                  src={
                    show ? "/images/hamburger2.png" : "/images/hamburger.png"
                  }
                  alt={show ? "Close menu" : "Open menu"}
                  width={21}
                  height={21}
                  className={`z-[999] ${
                    show ? "rotate-90 w-[18px]" : "rotate-0 w-5"
                  } transition-all duration-300 ease-linear cursor-pointer`}
                />
              </div>
              <nav
                ref={hamburgerMenuRef}
                className={`absolute -top-2.5 -left-3.5 border border-white shadow-2xl bg-gradient-to-r from-[#3E2FE1] to-[#8C14AC] z-[999] rounded-b-xl rounded-tr-xl ${
                  show ? "w-[230px] h-[430px]" : "w-0 h-0"
                } transition-all duration-300 ease-linear overflow-hidden`}
                aria-hidden={!show}
              >
                <ul className="text-white text-[1.1rem] py-16 px-3 space-y-4 flex flex-col z-[999]">
              ...
              <div
                className="relative text-black search-container flex gap-2 items-center"
